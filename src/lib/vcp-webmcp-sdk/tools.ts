...
const DEFAULT_PERSONAS: PersonaInfo[] = [
	{ id: 'muse', name: 'Muse', description: 'Creative and inspiring. Adapts to energy and constraints.', use: 'Learning, exploration, creative work' },
	...
	return async (args) => {
		...
		if (typeof window !== 'undefined') {
			window.dispatchEvent(
				new CustomEvent('webmcp:tool-call', { detail: { tool: name, timestamp: Date.now() } })
			);
		}
		return fn(args);
	};
	...
	return {
		...
		inputSchema: {
			...
			properties: {
				...
				vcp_context: {
					type: 'object',
					description: 'Optional VCP context object with personal_state, constraints, public_profile, etc.'
				},
				constitution_id: {
					type: 'string',
					description: "Constitution ID (e.g. 'personal.growth.creative'). Defaults to 'general'."
				},
				persona: {
					type: 'string',
					enum: personaIds,
		...
		execute: wrapExecute('vcp_chat', async (args) => {
			...
			const body = {
				...
				constitution_id: typeof args.constitution_id === 'string' ? args.constitution_id : 'general',
			...
			try {
				...
				if (contentType.includes('application/json')) {
					const json = (await response.json()) as { fallback?: boolean; reason?: string; message?: string };
					if (json.fallback) {
						return textResult(json.message ?? `Chat unavailable: ${json.reason ?? 'unknown'}`);
					}
				}
	...
	return {
		...
		inputSchema: {
			...
			properties: {
				vcp_context: {
					type: 'object',
					description: 'VCP context object. Must include at least vcp_version, profile_id, and constitution (with id and version).'
				}
			},
			required: ['vcp_context']
		},
		annotations: { readOnlyHint: true },
		execute: wrapExecute('vcp_build_token', async (args) => {
...
function createParseTokenTool(config: VCPWebMCPConfig): WebMCPToolDefinition | null {
	...
	return {
		...
		inputSchema: {
			type: 'object',
			properties: {
				token: { type: 'string', description: 'CSM-1 token string (newline-separated lines)' }
			},
			required: ['token']
		},
		annotations: { readOnlyHint: true },
		execute: wrapExecute('vcp_parse_token', async (args) => {
...
function createSummaryTool(config: VCPWebMCPConfig): WebMCPToolDefinition | null {
	...
	return {
		...
		annotations: { readOnlyHint: true },
		execute: wrapExecute('vcp_transmission_summary', async (args) => {
			if (!isRecord(args.vcp_context)) return errorResult('vcp_context must be an object');
			...
			try {
				...
				const lines = [
					...
					...(summary.transmitted.length > 0 ? summary.transmitted.map((f) => `- ${f}`) : ['- (none)']),
					'',
					'### Withheld (private, never sent)',
					...(summary.withheld.length > 0 ? summary.withheld.map((f) => `- ${f}`) : ['- (none)']),
					'',
					'### Influencing (shapes response without exposing raw data)',
					...(summary.influencing.length > 0 ? summary.influencing.map((f) => `- ${f}`) : ['- (none)'])
				];
...
function createPersonasTool(config: VCPWebMCPConfig): WebMCPToolDefinition {
	...
	return {
		...
		execute: wrapExecute('vcp_list_personas', async () => {
			const lines = [
				'## VCP Personas',
				'',
				'| ID | Name | Description | Use Cases |',
				'|---|---|---|---|',
				...personas.map((p) => `| ${p.id} | ${p.name} | ${p.description} | ${p.use} |`)
			];
			return textResult(lines.join('\n'));
		}, config.onToolCall)
	};
